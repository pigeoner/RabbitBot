...
class insert_redis:
    ...
    async def update_list_add(self, event_id, dict_data):
        self.redis_conn.hset(name=event_id, key=event_id, value=json.dumps(dict_data))
        self.redis_conn.expire(event_id, 259200)
        self.redis_conn.close()

    async def update_list_pass(self, event_id: str):
        back = self.redis_conn.hget(name=event_id, key=event_id)
        self.redis_conn.hdel(event_id, event_id)
        self.redis_conn.close()
        print(back)
        return json.loads(back) if back else None
