...
class zplay:
    ...
    async def update(self):
        event_id = self.tool.update_event_id()
        if any(event_id):
            event_id = event_id[0]
            data = await self.redis.update_list_pass(event_id=event_id)
        else:
            data = None
        if data:
            if self.arg == 't':
                code = 200
                print(data["bv"])
                dl = download_bili(data["bv"])
                data_user = data["uid"]
                await dl.download_video_bili(url=await dl.url_main())
            else:
                code = 400
                data_user = data["uid"]
        else:
            code = 300
            data_user = '该请求已被审核，或是已过期'
        return {"code": code, "uid": data_user, "event_id": event_id}
...
class zplay_tool:
    ...
    def update_event_id(self):
        return re.findall('event_id\((.*)\)', str(self.event.reply.message))
